...
function GamePage() {
  ...
  const handleUpdateDiscordId = () => {
    const trimmedId = discordId.trim();

    // Save to localStorage
    if (trimmedId) {
      localStorage.setItem('discordId', trimmedId);
    } else {
      localStorage.removeItem('discordId');
    }

    // Send to server
    socket.emit('set_discord_id', { discordId: trimmedId || null });

    toast.success(trimmedId ? 'Discord ID updated!' : 'Discord ID removed');
  };
        ...
        <div className="game-main">
          {gameState.phase === 'waiting' ? (
            <div className="waiting-room">
              ...
              {/* Discord ID Section */}
              <div className="discord-id-section" style={{
                marginTop: '1.5rem',
                padding: '1rem',
                ...
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '0.5rem' }}>
                  <svg width="20" height="20" viewBox="0 0 71 55" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ marginRight: '0.5rem' }}>
                  ...
                  <h3 style={{ margin: 0, fontSize: '1rem', color: '#fff' }}>Discord Voice Chat</h3>
                </div>
                <p style={{ fontSize: '0.85rem', color: '#b0b0b0', marginBottom: '1rem' }}>
                  Link your Discord account for automatic voice muting during game phases.
                </p>
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <input
                      ...
                      placeholder="Discord User ID (18 digits)"
                      value={discordId}
