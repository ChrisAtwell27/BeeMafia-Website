 ...
 * Imported from Discord Bot
 *
 * Role Structure:
 * - name: Display name of the role
 * - emoji: Emoji representation
 * - team: 'bee', 'wasp', or 'neutral'
 * - subteam: For neutrals - 'killing', 'evil', 'benign', or 'chaos'
...
const ROLES = {
    ...
    TRANSPORTER_BEE: {
        ...
        winCondition: 'Eliminate all Wasps and harmful Neutrals',
        nightAction: true, // Changed from false for web app
    ...
    MUTE_WASP: {
        ...
        emoji: '🗡️',
        team: 'wasp',
        description: 'You are a **Mute Wasp**! You carry out the kills for the Wasp team. You are permanently muted and all messages are turned into emojis!',
        abilities: ['Kill target chosen by Wasps', 'Basic attack', 'Become Wasp Queen if Queen dies', 'Communicate with Wasps', 'Server muted - all text messages translated to emojis'],
        winCondition: 'Equal or outnumber all other players',
        nightAction: true,
        actionType: 'mafia_kill',
        attack: 1,
    ...
    BUTTERFLY: {
        ...
        abilities: ['Put on a vest 3 times for powerful defense'],
    ...
    MUTE_BUTTERFLY: {
        ...
        abilities: ['Put on a vest 3 times for powerful defense', 'Server muted - all text messages translated to emojis'],
        winCondition: 'Survive to the end of the game',
        nightAction: true,
        actionType: 'vest',
        attack: 0,
...
// Role distribution templates for different game sizes
const ROLE_SETUPS = {
    basic: {
        6: ['QUEENS_GUARD', 'NURSE_BEE', 'WORKER_BEE', 'WORKER_BEE', 'WASP_QUEEN', 'KILLER_WASP'],
        8: ['QUEENS_GUARD', 'SCOUT_BEE', 'NURSE_BEE', 'GUARD_BEE', 'WORKER_BEE', 'WASP_QUEEN', 'KILLER_WASP', 'BUTTERFLY'],
        10: ['QUEENS_GUARD', 'SCOUT_BEE', 'NURSE_BEE', 'GUARD_BEE', 'LOOKOUT_BEE', 'WORKER_BEE', 'WASP_QUEEN', 'KILLER_WASP', 'SPY_WASP', 'BUTTERFLY'],
        12: ['QUEENS_GUARD', 'SCOUT_BEE', 'NURSE_BEE', 'GUARD_BEE', 'LOOKOUT_BEE', 'TRACKER_BEE', 'WORKER_BEE', 'WORKER_BEE', 'WASP_QUEEN', 'KILLER_WASP', 'SPY_WASP', 'CLOWN_BEETLE'],
        15: ['QUEENS_GUARD', 'SCOUT_BEE', 'NURSE_BEE', 'GUARD_BEE', 'LOOKOUT_BEE', 'TRACKER_BEE', 'SOLDIER_BEE', 'WORKER_BEE', 'WORKER_BEE', 'WORKER_BEE', 'WASP_QUEEN', 'KILLER_WASP', 'SPY_WASP', 'CLOWN_BEETLE', 'BUTTERFLY']
    },
    chaos: {
        6: ['SCOUT_BEE', 'NURSE_BEE', 'SOLDIER_BEE', 'KILLER_WASP', 'MURDER_HORNET', 'BUTTERFLY'],
        8: ['QUEENS_GUARD', 'SCOUT_BEE', 'NURSE_BEE', 'GUARD_BEE', 'WASP_QUEEN', 'KILLER_WASP', 'MURDER_HORNET', 'CLOWN_BEETLE'],
        10: ['QUEENS_GUARD', 'SCOUT_BEE', 'NURSE_BEE', 'GUARD_BEE', 'LOOKOUT_BEE', 'SOLDIER_BEE', 'WASP_QUEEN', 'KILLER_WASP', 'MURDER_HORNET', 'BUTTERFLY']
    }
};

// Helper to get role setup based on player count
function getRoleSetup(gameMode, playerCount) {
    const mode = ROLE_SETUPS[gameMode] || ROLE_SETUPS.basic;

    // Find the closest setup for the player count
    const counts = Object.keys(mode).map(Number).sort((a, b) => a - b);
    ...
    const setup = [...mode[selectedCount]];

    // Pad with worker bees if we need more roles
    while (setup.length < playerCount) {
        setup.push('WORKER_BEE');
    }
